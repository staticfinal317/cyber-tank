import { AMMO, cloneLoadout, MOVEMENT_MODULES, PAINTS, SEASONS, TOOLS } from '../content/expedition';
import type { AmmoId, LoadoutPreset, MovementModuleId, PaintId, SaveData, TankLoadout, ToolId } from '../core/types';

type Category = 'movement' | 'ammo' | 'tool' | 'appearance';

export interface WorkshopActions {
  preview: (loadout: TankLoadout) => void;
  save: (preset: LoadoutPreset) => void;
  testDrive: (loadout: TankLoadout) => void;
  launch: (loadout: TankLoadout) => void;
  close: () => void;
...
export class WorkshopController {
  ...
  constructor(private readonly actions: WorkshopActions) {
    ...
    node('workshop-test').addEventListener('click', () => { if (this.loadout) actions.testDrive(cloneLoadout(this.loadout)); });
    node('workshop-launch').addEventListener('click', () => { if (this.loadout) actions.launch(cloneLoadout(this.loadout)); });
  ...
  open(save: SaveData): void {
    ...
    this.renderAll(); this.actions.preview(cloneLoadout(this.loadout));
  }
  ...
  private renderAll(): void {
    ...
    this.renderPresets(); this.renderItems(); this.renderDetails(); this.renderAmmoSlots();
  }
  ...
  private renderItems(): void {
    ...
    const add = (id: string, name: string, description: string, color: number, selected: boolean, unlocked: boolean, cost: number, apply: () => void) => {
      ...
      button.innerHTML = `<i class="ph ${icon} module-visual"></i><span>${name}</span><small>${unlocked ? description : `${cost} 星屑解锁`}</small>${selected ? '<em>已安装</em>' : ''}`;
      const choose = async () => {
        if (!unlocked) { if (await this.actions.unlock(this.category, id, cost)) { this.renderItems(); } return; }
        apply(); this.dirty = true; this.renderItems(); this.renderDetails(); this.renderAmmoSlots(); this.actions.preview(cloneLoadout(this.loadout!));
      };
