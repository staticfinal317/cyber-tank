...
import {
  ...
  maxPlayerScore,
  nextMenuSelection,
} from '../src/classic/game/ClassicGame';
import { Dir, type PlayerHudView, type PlayerInput, type SimEvent, type TankView } from '../src/classic/core/types';
...
function enemyDestroyed(kind: 'basic' | 'fast' | 'power' | 'armor', score: number, byPlayer = 0): SimEvent {
  return { type: 'enemyDestroyed', tankId: 1, kind, score, x: 0, y: 0, byPlayer };
}
...
/* ==================== ClassicGame.ts：StageKillTally 双人分账（D3） ==================== */

describe('StageKillTally · 双人分账（D3：按 enemyDestroyed.byPlayer 隔离计数/分值）', () => {
  it('不同 playerIndex 的击杀各自独立累计，互不干扰', () => {
    const tally = new StageKillTally();
    tally.applyEvents([
      enemyDestroyed('basic', 100, 0),
      enemyDestroyed('basic', 100, 1),
      enemyDestroyed('fast', 200, 1),
    ]);
    expect(tally.countOf('basic', 0)).toBe(1);
    expect(tally.scoreOf('basic', 0)).toBe(100);
    expect(tally.countOf('basic', 1)).toBe(1);
    expect(tally.scoreOf('basic', 1)).toBe(100);
    expect(tally.countOf('fast', 1)).toBe(1);
    expect(tally.countOf('fast', 0)).toBe(0); // 未影响 P1
  });

  it('playerIndex 默认 0：兼容既有单参数调用（1P 场景语义不变）', () => {
    const tally = new StageKillTally();
    tally.applyEvents([enemyDestroyed('armor', 400, 0)]);
    expect(tally.countOf('armor')).toBe(1); // 不传 playerIndex 即读 P1（下标 0）
    expect(tally.scoreOf('armor')).toBe(400);
  });

  it('breakdownRows(playerIndex)：固定按 basic/fast/power/armor 顺序，且各玩家统计独立', () => {
    const tally = new StageKillTally();
    tally.applyEvents([enemyDestroyed('fast', 200, 0), enemyDestroyed('power', 300, 1)]);
    expect(tally.breakdownRows(0).map((row) => row.kind)).toEqual(['basic', 'fast', 'power', 'armor']);
    expect(tally.breakdownRows(0).find((row) => row.kind === 'fast')).toEqual({
      kind: 'fast',
      count: 1,
      unitPrice: 200,
      subtotal: 200,
    });
    expect(tally.breakdownRows(1).find((row) => row.kind === 'power')).toEqual({
      kind: 'power',
      count: 1,
      unitPrice: 300,
      subtotal: 300,
    });
    expect(tally.breakdownRows(1).find((row) => row.kind === 'fast')?.count).toBe(0); // P2 未击杀 fast
  });

  it('reset()：两个玩家的计数与分值同时清零', () => {
    const tally = new StageKillTally();
    tally.applyEvents([enemyDestroyed('basic', 100, 0), enemyDestroyed('basic', 100, 1)]);
    tally.reset();
    expect(tally.countOf('basic', 0)).toBe(0);
    expect(tally.countOf('basic', 1)).toBe(0);
  });
});

/* ==================== ClassicGame.ts：D1/D2/D4 纯逻辑辅助函数（可独立单测，无 DOM 依赖） ==================== */

describe('nextMenuSelection · 菜单光标翻转（D1）', () => {
  it('1 → 2，2 → 1：仅两个选项，直接取反', () => {
    expect(nextMenuSelection(1)).toBe(2);
    expect(nextMenuSelection(2)).toBe(1);
  });
});

describe('buildStepInputs · D2 输入路由', () => {
  const kb: PlayerInput = { dir: Dir.Left, fire: true };
  const gp: PlayerInput = { dir: Dir.Right, fire: false };

  it('1P：维持现状合并——dir 手柄优先于键盘，fire 取或', () => {
    expect(buildStepInputs(1, kb, gp)).toEqual([{ dir: Dir.Right, fire: true }]);
    expect(buildStepInputs(1, kb, { dir: null, fire: false })).toEqual([{ dir: Dir.Left, fire: true }]); // 手柄无方向：回退键盘
    expect(buildStepInputs(1, { dir: null, fire: false }, { dir: null, fire: false })).toEqual([
      { dir: null, fire: false },
    ]);
  });

  it('2P：键盘/手柄各自独立驱动 P1/P2，不合并', () => {
    expect(buildStepInputs(2, kb, gp)).toEqual([kb, gp]);
  });

  it('2P：手柄未连接（中性输入）时 P2 自然为中性，不影响 P1 的键盘输入', () => {
    const neutralGp: PlayerInput = { dir: null, fire: false };
    expect(buildStepInputs(2, kb, neutralGp)).toEqual([kb, neutralGp]);
  });
});

describe('buildNeutralInputs · 结算延迟期喂给 world.tick() 的中性输入数组', () => {
  it('1P：长度 1；2P：长度 2；元素均为 { dir: null, fire: false }', () => {
    expect(buildNeutralInputs(1)).toEqual([{ dir: null, fire: false }]);
    expect(buildNeutralInputs(2)).toEqual([
      { dir: null, fire: false },
      { dir: null, fire: false },
    ]);
  });
...
describe('buildWorldOptions · 由 menuSelection 派生 World 构造参数', () => {
  it('playerCount 取自 menuSelection，level/rng/carryOver 原样透传', () => {
    const level = CLASSIC_LEVELS[0];
    if (!level) throw new Error('测试夹具异常：CLASSIC_LEVELS 缺少第 1 关');
    const rng = new RNG(1);
    const carryOver = [{ level: 2, lives: 3, score: 500, out: false }];
    expect(buildWorldOptions(level, rng, 2, carryOver)).toEqual({ level, rng, playerCount: 2, carryOver });
    expect(buildWorldOptions(level, rng, 1, undefined)).toEqual({
      level,
      rng,
      playerCount: 1,
      carryOver: undefined,
...
describe('buildCarryOver · 过关时构造下一关的各玩家 carryOver（D4）', () => {
  function fakeTank(overrides: Partial<TankView>): TankView {
    return {
      id: 0,
      kind: 'player',
      playerIndex: 0,
  ...
  it('在场玩家：level 取自对应坦克，lives/score/out 取自 hud', () => {
    const players: PlayerHudView[] = [
      { lives: 2, score: 1000, out: false },
      { lives: 1, score: 500, out: false },
    ];
    const tanks: TankView[] = [fakeTank({ playerIndex: 0, level: 3 }), fakeTank({ playerIndex: 1, level: 1 })];
    expect(buildCarryOver(players, tanks)).toEqual([
      { level: 3, lives: 2, score: 1000, out: false },
      { level: 1, lives: 1, score: 500, out: false },
    ]);
  });

  it('出局/不在场玩家：找不到对应坦克时 level 兜底为 0', () => {
    const players: PlayerHudView[] = [
      { lives: 0, score: 300, out: true },
      { lives: 2, score: 800, out: false },
    ];
    const tanks: TankView[] = [fakeTank({ playerIndex: 1, level: 2 })]; // 只有 P2 在场
    expect(buildCarryOver(players, tanks)).toEqual([
      { level: 0, lives: 0, score: 300, out: true }, // P1 出局：level 兜底 0
      { level: 2, lives: 2, score: 800, out: false },
    ]);
  });

  it('单人模式：players 长度 1，行为与既有 1P carryOver 逻辑一致', () => {
    const players: PlayerHudView[] = [{ lives: 3, score: 1200, out: false }];
    const tanks: TankView[] = [fakeTank({ playerIndex: 0, level: 2 })];
    expect(buildCarryOver(players, tanks)).toEqual([{ level: 2, lives: 3, score: 1200, out: false }]);
  });
});

describe('maxPlayerScore · HI-SCORE 与结算总分展示统一取各玩家最高分（D4）', () => {
  it('1P：即该玩家自身分数，行为不变', () => {
    expect(maxPlayerScore([{ lives: 3, score: 777, out: false }])).toBe(777);
  });

  it('2P：取两人中较高分，与数组顺序无关', () => {
    const higherFirst: PlayerHudView[] = [
      { lives: 0, score: 900, out: true },
      { lives: 3, score: 500, out: false },
    ];
    const higherSecond: PlayerHudView[] = [
      { lives: 3, score: 500, out: false },
      { lives: 0, score: 900, out: true },
    ];
    expect(maxPlayerScore(higherFirst)).toBe(900);
    expect(maxPlayerScore(higherSecond)).toBe(900);
  });

  it('空数组兜底：返回 0（实际流程中不应发生，仅防御）', () => {
    expect(maxPlayerScore([])).toBe(0);
  });
});
