...
/* ==================== GamepadMapper：menuUp/menuDown 边沿检测（D1） ==================== */

describe('GamepadMapper · menuUp/menuDown 边沿检测（D1：十字键+摇杆并集上沿，持续按住不重复，松开后重触发）', () => {
  it('十字键"上/下"从无到有的上沿各产出一次 menuUp/menuDown，按住不重复', () => {
    const mapper = new GamepadMapper();
    expect(mapper.update(frame({ 12: true }))).toEqual(['menuUp']); // 上：新上沿
    expect(mapper.update(frame({ 12: true }))).toEqual([]); // 按住不重复
    expect(mapper.update(frame({ 13: true }))).toEqual(['menuDown']); // 松开上、按下下：新上沿
    expect(mapper.update(frame({ 13: true }))).toEqual([]); // 按住不重复
  });

  it('松开后再次按下：重新触发上沿', () => {
    const mapper = new GamepadMapper();
    expect(mapper.update(frame({ 12: true }))).toEqual(['menuUp']);
    expect(mapper.update(frame({}))).toEqual([]); // 松开，非上沿
    expect(mapper.update(frame({ 12: true }))).toEqual(['menuUp']); // 再次按下：新上沿
  });

  it('摇杆推"上/下"同样触发上沿（十字键∪摇杆的并集语义）', () => {
    const mapper = new GamepadMapper();
    expect(mapper.update(frame({}, [0, -0.8]))).toEqual(['menuUp']); // 摇杆推上
    expect(mapper.update(frame({}, [0, -0.8]))).toEqual([]); // 持续推着不重复
    expect(mapper.update(frame({}, [0, 0.8]))).toEqual(['menuDown']); // 摇杆推下：新上沿
  });

  it('十字键与摇杆同指一个方向：并集语义下一方松开、另一方仍激活时不产生新上沿', () => {
    const mapper = new GamepadMapper();
    expect(mapper.update(frame({ 12: true }, [0, -0.8]))).toEqual(['menuUp']); // 十字键+摇杆都按"上"，仅一次上沿
    expect(mapper.update(frame({}, [0, -0.8]))).toEqual([]); // 松开十字键，摇杆仍按"上"：Up 仍活跃，非上沿
    expect(mapper.update(frame({}, [0, 0]))).toEqual([]); // 摇杆回中：Up 变为不活跃，是"释放"而非"按下"，非上沿
    expect(mapper.update(frame({ 12: true }))).toEqual(['menuUp']); // 重新按下：新上沿
  });

  it('menuUp/menuDown 与 confirm/pause 互不干扰，可在同一帧同时产出', () => {
    const mapper = new GamepadMapper();
    const actions = mapper.update(frame({ 12: true, 0: true, 9: true }));
    expect(actions).toEqual<MenuAction[]>(['menuUp', 'confirm', 'pause']);
  });

  it('update(null) 断开后重连：边沿检测从"全松开"重新开始，仍按住的方向会重新触发', () => {
    const mapper = new GamepadMapper();
    expect(mapper.update(frame({ 12: true }))).toEqual(['menuUp']);
    expect(mapper.update(null)).toEqual([]);
    expect(mapper.update(frame({ 12: true }))).toEqual(['menuUp']); // 视为全松开后的新上沿
  });
});
...
describe('GamepadBridge · 浏览器桥接（注入假 getGamepads + 假事件 target）', () => {
  ...
  it('poll 后 sample() 返回归一化的 PlayerInput，且边沿动作经 onMenuAction 上抛', () => {
    ...
    const pads: readonly (Gamepad | null)[] = [fakeGamepad([15, 0])]; // 十字键"右" + 面键 0（fire & confirm 上沿）
    const bridge = new GamepadBridge({
    ...
    bridge.poll();
    expect(bridge.sample()).toEqual({ dir: Dir.Right, fire: true });
